                              ...
                              key* keysIn,  val* valuesIn)
{
    ...
    uint4 key, value;
    uint4 *key_arr, *value_arr;
    uint4 *key_out_arr, *value_out_arr;
    key_arr = (uint4 *)keysIn[i].keys;
    value_arr = (uint4 *)valuesIn[i].vals;
    key_out_arr = (uint4 *)keysOut[i].keys;
    value_out_arr = (uint4 *)valuesOut[i].vals;
    key = key_arr[0];
    value = value_arr[0];

//    radixSortBlocksInternal(nbits, startbit, (uint4 *)keysOut[i].keys, (uint4 *)valuesOut[i].vals,
//                    (uint4 *)keysIn[i].keys, (uint4 *)valuesIn[i].vals, tid, localSize);

//    uint temp_key, temp_val;
//    temp_key = keysIn[i].keys[0];
//    temp_val = valuesIn[i].vals[0];
//    key = make_uint4(temp_key, temp_key, temp_key, temp_key);
//    value = make_uint4(temp_val, temp_val, temp_val, temp_val);
//    key = *reinterpret_cast<uint4 *>(keysIn[i].keys[0]);
//    value = *reinterpret_cast<uint4 *>(valuesIn[i].vals[0]);
...
//    *reinterpret_cast<uint4 *>(keysOut[i].keys[0])   = key;
//    *reinterpret_cast<uint4 *>(valuesOut[i].vals[0]) = value;
    key_out_arr[0] = key;
    value_out_arr[0] = value;
//    keysOut[i].keys[0] = key.x;
//    valuesOut[i].vals[0] =value.x;
//    if (keysOut[i].keys[0] != 0)
//        printf("changed %d\n", keysOut[i].keys[0]);
}
